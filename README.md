...
[![Tests](https://img.shields.io/badge/Tests-60%2F60_Passing-brightgreen?style=for-the-badge&logo=vitest&logoColor=white)](#receipts)
...
* 🎯 **60 / 60 Tests Passing**: 100% test pass rate across 8 workspace packages (`types`, `config`, `market-engine`, `risk-engine`, `agent`, `solana`, `api`, `web`).
...
* ⛓️ **Solana Devnet Attestation**: Every user-approved trade signs an on-chain transaction recorded via the official SPL Memo Program (`MemoSq4gqABAXKb96qnH8TysNcWxMyWCqXgDLGmfcHr`) with clickable Solscan links.
...
| **Testing** | Vitest, ESLint, TypeScript compiler | 60 unit, integration, and security regression tests |
...
pnpm run verify
```
Runs lint, TypeScript typechecking, all 60 tests, and Next.js production build.
...
* **Devnet Attestation Scope**: Real transactions sign and commit to Solana Devnet via the SPL Memo program. Tokenized stock liquidity pools are testnet-scale.
