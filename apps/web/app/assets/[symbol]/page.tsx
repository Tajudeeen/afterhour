  ...
  return (
    <div className="page-shell">
      ...
      <div className="grid-2" style={{ marginTop: '16px' }}>
        <div>
          <div className="data-card">
            <h3>Onchain price</h3>
            <div className="data-value">{formatCurrency(intelligence.onchainPrice, 2)}</div>
        ...
        <div>
          <div className="data-card">
            ...
            <div className="data-value">{formatCurrency(intelligence.referencePrice, 2)}</div>
            <div className="data-label" style={{ fontFamily: 'monospace', fontSize: '0.72rem' }}>
            ...
            {intelligence.pythConfidenceUsd != null && (
              <div style={{ marginTop: '4px', fontSize: '0.74rem', color: 'var(--pyth-lavender)', fontFamily: 'SF Mono, monospace' }}>
                Pyth Band: ±{formatCurrency(intelligence.pythConfidenceUsd, 2)} ({formatNum(intelligence.pythConfidenceRatioPercent, 2)}%)
              </div>
            )}
          </div>
        </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px' }}>
            <div>
              ...
              <div style={{ fontFamily: 'SF Mono, monospace', fontSize: '1.1rem', fontWeight: 700, color: 'var(--ink-heading)', marginTop: '2px' }}>
                {formatCurrency(intelligence.pythFeedPair.equityPrice, 2)}
              </div>
            ...
            <div>
              ...
              <div style={{ fontFamily: 'SF Mono, monospace', fontSize: '1.1rem', fontWeight: 700, color: 'var(--ink-heading)', marginTop: '2px' }}>
                {formatCurrency(intelligence.pythFeedPair.tokenPrice, 2)}
              </div>
              <div style={{ fontSize: '0.72rem', color: 'var(--ink-subtle)', fontFamily: 'SF Mono, monospace', marginTop: '2px' }}>
            ...
            <div>
              ...
              <div style={{ fontFamily: 'SF Mono, monospace', fontSize: '1.1rem', fontWeight: 800, color: (intelligence.pythFeedPair.gapPercent ?? 0) > 0 ? 'var(--lime)' : 'var(--amber)', marginTop: '2px' }}>
                {formatPercent(intelligence.pythFeedPair.gapPercent, 2)}
              </div>
      ...
      <div className="grid-3" style={{ marginTop: '24px' }}>
        <div className="data-card">
          <h3>Gap</h3>
          <div className={`data-value ${(intelligence.gapPercent ?? 0) > 0 ? 'gap-positive-large' : 'gap-negative-large'}`}>
            {formatPercent(intelligence.gapPercent, 2)}
          </div>
          <div className="data-label">
            {(intelligence.gapDollar ?? 0) > 0 ? '+' : ''}${Math.abs(intelligence.gapDollar ?? 0).toFixed(2)} per token
          </div>
        </div>
        <div className="data-card">
        ...
        <div className="data-card">
          <h3>Risk score</h3>
          <div className="data-value" style={{ color: 'var(--lime)' }}>{intelligence.riskScore?.score ?? 0}</div>
          <div className="data-label">{intelligence.riskScore?.band ?? 'Normal'}</div>
        </div>
      </div>
      ...
      {intelligence.routes && intelligence.routes.length > 0 && (
        <div style={{ marginTop: '32px' }}>
          ...
          <div className="route-card route-best">
            ...
            <div className="route-price">{formatCurrency(intelligence.routes[0]!.price, 2)} / token</div>
            <div className="route-meta">
              <span>Impact: <strong>{formatNum(intelligence.routes[0]!.priceImpact, 2)}%</strong></span>
              <span>Fees: <strong>{formatCurrency(intelligence.routes[0]!.fees, 2)}</strong></span>
